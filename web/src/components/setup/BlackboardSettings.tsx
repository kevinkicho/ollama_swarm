...
export function BlackboardWallClockCap({
  ...
  const isValid = trimmed.length === 0 || (Number.isFinite(parsed) && parsed >= 0 && parsed <= 480);
  return (
    <Field
      label="Wall-clock cap minutes (Unit 43)"
      hint={
        trimmed.length === 0 || parsed === 0
          ? "0 or empty → uses the server default (8-hour baked-in). Enter a number 1–480 to cap THIS run only."
          : isValid
            ? `Run will stop after ~${parsed} min of active wall-clock time (host-sleep is still clamped per Unit 27).`
            : "Out of range — enter a number between 1 and 480, or leave empty/0 for the server default."
      }
    >
      <input
        value={wallClockCapMin}
        onChange={(e) => setWallClockCapMin(e.target.value)}
        placeholder="(default: 0 = server default)"
...
// NEW: auditor control flags
export function AuditorControlFlags({
    ...
    >
      <div className="space-y-3 pt-1">
        <ToggleField
          ...
          onChange={setAuditorOnlyMutations}
          hint="Workers propose only; auditor reviews + commits (batched)."
        />
        <ToggleField
          label="Require auditor verification"
          checked={requireAuditorVerification}
          onChange={setRequireAuditorVerification}
          hint="Force verifyCommand to run in the auditor's batch apply (even if not configured for workers)."
        />
        <div className="text-[10px] text-ink-500">
          Recommended for high-safety runs. The final commit message will attribute the batch to the auditor.
        </div>
      </div>
    </Field>
  );
...
// Hybrid / Planning Phase control (items #1 #2 in implementation plan).
// Dedicated section under blackboard advanced. Uses Field + ToggleField for polish/consistency.
// Only shown for blackboard preset. When enabled, Orchestrator builds a PipelineRunner that
// runs planningPreset first (council recommended for debate + broad view) then pipes the
// deliverable + transcript snippets as directive/context into the executionPreset (blackboard).
// This gives "god-mode discreetly" in planning without weakening blackboard's gated execution.
export function PlanningPhaseControl({
  ...
  return (
    <Field
      label="🧭 Planning Phase (Hybrid)"
      labelAccessory={<InfoTip>Broad planning (e.g. council debate/synthesis) builds systemic understanding + deliverable, then automatically pipes as directive + transcript into blackboard for safe, gated execution + one batch commit. Combines strengths of other presets for planning without touching blackboard limits.</InfoTip>}
    >
      <div className="space-y-2 pt-1">
        <ToggleField
          label="Hybrid: Broad Planning → Robust Execution"
          labelAccessory={<InfoTip>Planning phase runs first; results feed blackboard execution (auditor reviews + single batched commit).</InfoTip>}
          checked={useHybridPlanning}
          onChange={setUseHybridPlanning}
        />
        {useHybridPlanning && (
          <div className="ml-6 space-y-2 text-xs">
            <div>
              <span className="text-ink-400">Planning preset: </span>
              <select
                value={planningPreset || "council"}
                onChange={(e) => setPlanningPreset(e.target.value)}
                className="bg-ink-900 border border-ink-700 px-1.5 py-0.5 rounded text-ink-200"
              >
                <option value="council">council (recommended: debate + big-picture synthesis)</option>
                <option value="stigmergy">stigmergy (free exploration + pheromone trails, good for discovery)</option>
            ...
            <div>
              <span className="text-ink-400">Execution preset: </span>
              <select
                value={executionPreset || "blackboard"}
                onChange={(e) => setExecutionPreset(e.target.value)}
                className="bg-ink-900 border border-ink-700 px-1.5 py-0.5 rounded text-ink-200"
              >
                <option value="blackboard">blackboard (auditor-gated, batch commit)</option>
              </select>
            </div>
            <div className="flex items-center gap-1 text-ink-500 text-[10px]">
              Pipeline flow
              <InfoTip>Flow: [Pipeline] phase 1/2 (planning) → piped context → phase 2/2 blackboard planner uses rich context + systemMap for TODOs → workers propose → auditor explicit reviewProposedHunks + in-memory batch apply → one git commit.</InfoTip>
            </div>
          </div>
        )}
        {/* Web research tools toggle - next to hybrid for research planning directives */}
        <div className="mt-2 pt-2 border-t border-ink-700/50">
          <ToggleField
            label="Enable web research tools (web_search + web_fetch)"
            labelAccessory={<InfoTip>Gives the planner internet search & fetch tools (via DuckDuckGo + bounded fetch) — **completely free and keyless, no MCP config needed**. Use for directives like 'discover governmental/intra-governmental data API endpoints'. Results truncated for safety; model follows up on URLs. No blanket internet for workers. (MCP servers field is only for *additional* tools.)</InfoTip>}
            checked={!!webTools}
            onChange={setWebTools || (() => {})}
          />
        </div>
        {/* MCP servers surface */}
        <div className="mt-2">
          <Field
            label="MCP Servers (experimental)"
            labelAccessory={<InfoTip>Spawn/connect MCP servers for more tools (e.g. advanced fetch, github, custom gov data). Format: name=command. Free keyless search: search=npx -y open-websearch@latest (multi-engine DDG/Bing/etc, no key). Alternatives: pskill9/web-search or heventure-search-mcp (uvx). Available to research profile.</InfoTip>}
          >
            <input
              value={mcpServers || ""}
              onChange={(e) => setMcpServers && setMcpServers(e.target.value)}
              placeholder="search=npx -y open-websearch@latest fetch=npx -y @modelcontextprotocol/server-fetch"
            ...
            <button
              ...
              title="Adds the recommended completely free & keyless multi-engine search MCP (no API key). Also enables the native web research tools."
            >
