...
// 2026-04-27: video capture via Playwright's recordVideo (CDP
// screencast under the hood — same protocol Chrome DevTools uses).
// .webm at the viewport size; one file per session.
const VIDEO_DIR = path.join(PW_DIR, "video");
...
for (const d of [PW_DIR, SCREENSHOTS_DIR, DOM_DIR, VIDEO_DIR]) {
...
const ctx = await browser.newContext({
  ...
  // Video covers motion that 30s-tick screenshots miss (streaming
  // text arriving, agent transitions, board mutations). Periodic
  // screenshots stay as the inline-renderable spot-check; video is
  // the full replay.
  recordVideo: { dir: VIDEO_DIR, size: { width: 1600, height: 1200 } },
});
...
// Closing the context flushes the video file to disk. The path is
// auto-generated by Playwright; log it after close so the report has
// a stable pointer.
await ctx.close();
await browser.close();
// Find the recorded video file (Playwright auto-names them).
let videoPath = null;
try {
  const { readdir } = await import("node:fs/promises");
  const files = await readdir(VIDEO_DIR);
  const webm = files.find((n) => n.endsWith(".webm"));
  if (webm) videoPath = path.join(VIDEO_DIR, webm);
} catch {
  // VIDEO_DIR may be empty if recording never started
}
await rt("watcher_end", { terminalPhase, counts, eventTypeBreakdown: eventTypeCounts, videoPath });
