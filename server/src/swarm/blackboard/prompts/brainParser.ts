...
export type BrainPromptFn = (prompt: string, model: string, maxTokens: number, timeoutMs: number) => Promise<string>;
...
export async function brainFallbackParse<T>(
  ...
  try {
    const brainOutput = await promptFn(prompt, cfg.brainModel, cfg.maxTokens, cfg.timeoutMs);
